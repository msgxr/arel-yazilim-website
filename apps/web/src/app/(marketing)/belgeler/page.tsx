    ...
    <>
      <Section aria-labelledby="belgeler-heading">
        <div className="container-site">
          <SectionHeader
            label="Belgeler"
            title={<>Kulüp <span className="text-orange-DEFAULT">Belgeleri</span></>}
          ...
          <div className="mb-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {docs.map((doc) => (
              <a
                ...
                className="group flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-orange-DEFAULT hover:shadow-lg"
                aria-label={doc.external ? `${doc.title} (yeni sekmede açılır)` : doc.title}
              >
                <span className="text-3xl" aria-hidden="true">{doc.icon}</span>
                <div>
                  <div className="font-extrabold text-slate-800 group-hover:text-orange-DEFAULT transition-colors">{doc.title}</div>
